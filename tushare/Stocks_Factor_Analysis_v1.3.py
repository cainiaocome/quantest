...
def marketAnalysis(time_period=14):
    ...
    try:
        ...
        axes[2].axhline(y=0.3, c="red",linewidth=0.5,zorder=0)
        axes[2].set_ylim([0, 1])
    ...
    try:
        ...
        axes[3].set_title(u'成交量占比', fontproperties=font_large)
        axes[3].grid(color='white', which='both', linestyle='-', linewidth=0.4)
        axes[3].set_ylabel('成交量占M0百分比',fontproperties=font_medium)
        ...
        axes2 = axes[3].twinx()
        axes2.set_ylabel('成交量占M1百分比',fontproperties=font_medium)
...
def hindenburgOmen(index='000001'):
    path = '/Users/huiyang/Documents/quantest/tuShare/notification_monitoring_files/'
