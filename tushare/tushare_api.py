...
class tushare_api(object):
    ...
    def download_today_all(self, start='2017-01-01', end='2018-01-01'):
        """
        ...
        if stock_data is not None:
            ...
            stock_data['date_download'] = end
            filename = 'today_all'
            stock_data.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        else:
    ...
    def downloadIndex(self, start='2017-12-01', end='2018-06-01'):
        """
        ...
        if not index_000001.empty:
            ...
            index_000001.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
        if not index_399001.empty:
            filename = 'index_399001'
            index_399001.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
        if not index_399006.empty:
            filename = 'index_399006'
            index_399006.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        return
    ...
    def download_hist_data(self, start='2015-01-05', end='2015-01-09'):
        ...
        filename = 'today_all'
        today_all = pd.read_excel(io=self.working_folder  + filename + '.xlsx')
        ...
        filename = 'hist_data'
        stock_data_all.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
    ...
    def calculate_today_all_with_percentage(self, start='2015-01-05', end='2015-01-09'):
        ...
        filename = 'hist_data'
        hist_data = pd.read_excel(io=self.working_folder + filename + '.xlsx')
        ...
        today_all.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
    ...
    def calculate_roe_pb(self):
        ...
        for stk in today_all.index:
            ...
            if len(report_data_code) > 0:
                report_data_code.sort_values(by='report_date', ascending=True, inplace=True)
        ...
        #print('calculate_roe_pb successful')
        return


    def download_industry_classified(self,start='2017-01-01', end='2018-01-01'):
        """
        返回值说明：
        ...
        if industry_classified is not None:
            ...
            industry_classified['date_download'] = end
            filename = 'industry_classified'
            industry_classified.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        else:
            warning_code = pd.DataFrame(
                {'return code': ['for all stocks'], 'description': ['industry_classified download failed']})
            self.warning_list = self.warning_list.append(warning_code)

    def download_concept_classified(self,start='2017-01-01', end='2018-01-01'):
        """
        返回值说明：
        ...
        if concept_classified is not None:
            ...
            concept_classified.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        else:
            warning_code = pd.DataFrame(
                {'return code': ['for all stocks'], 'description': ['concept_classified download failed']})
            self.warning_list = self.warning_list.append(warning_code)

    def download_area_classified(self,start='2017-01-01', end='2018-01-01'):
        """
        返回值说明：
        ...
        if area_classified is not None:
            ...
            area_classified['date_download'] = end
            area_classified.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        else:
            warning_code = pd.DataFrame(
                {'return code': ['for all stocks'], 'description': ['concept_classified download failed']})
            self.warning_list = self.warning_list.append(warning_code)

    # get the current day stock basics
    def download_stock_basics(self,start='2017-01-01', end='2018-01-01'):
        """
        """
        ...
        if stock_basics is not None:
            ...
            stock_basics['date_download'] = end
            filename = 'stock_basics'
            stock_basics.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        else:
            warning_code = pd.DataFrame(
                {'return code': ['for all stocks'], 'description': ['stock_basics download failed']})
            self.warning_list = self.warning_list.append(warning_code)

    # get the history of stock basics
    def download_stock_basics_hist(self,start='2016-08-09', end='2018-07-01'):
        """
        """
        #print('downloading the stock_basics info...')
        total = 0
        stock_basics_hist = pd.DataFrame([])
        """
        """
        path = '/Users/yanghui/Documents/quantest/data_Shared/baoStock/'
        trading_days = pd.read_csv(path + "trade_dates.csv")
        trading_days = trading_days[(trading_days.calendar_date >= start)  \
                                    & (trading_days.calendar_date <= end)
                                    & (trading_days.is_trading_day == 1)]
        #trading_days.sort_values(by='calendar_date', ascending=False, inplace=True)
        for date in trading_days.calendar_date:
            try:
                stock_basics = ts.get_stock_basics(date=date)
                if not stock_basics.empty:
                    stock_basics['date'] = date
                    stock_basics_hist = stock_basics_hist.append(stock_basics)
            except:
                pass
        filename = 'stock_basics_hist'
        stock_basics_hist.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')


    @deprecated("This function is rotten, use 'download_stock_fundamentals' instead")
    def download_report_data(self, year=2017, quarter=3):
        ...
        try:
            ...
            data_2017_1.to_excel(self.working_folder + 'quantest/data_shared/' + filename + '.xlsx', encoding='GBK')
        except:
        ...
        try:
            ...
            data_2017_2.to_excel(self.working_folder + 'quantest/data_shared/' + filename + '.xlsx', encoding='GBK')
        except:
        ...
        try:
            ...
            data_2017_3.to_excel(self.working_folder + 'quantest/data_shared/' + filename + '.xlsx', encoding='GBK')
        except:
        ...
        try:
            ...
            data_2017_4.to_excel(self.working_folder + 'quantest/data_shared/' + filename + '.xlsx', encoding='GBK')
        except:
            print('download report_data_2017_4 failed')
        """

        try:
            data_2018_1 = ts.get_report_data(2018, 1)
            print('Now downloading report_data_2018_1, total records:' + str(len(data_2018_1)))
            filename = 'report_data_2018_1'
            data_2018_1.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        except:
            print('download report_data_2018_1 failed')

        try:
            data_2018_2 = ts.get_report_data(2018, 2)
            print('Now downloading report_data_2018_2, total records:' + str(len(data_2018_2)))
            filename = 'report_data_2018_2'
            data_2018_2.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
        except:
            print('download report_data_2018_2 failed')     

        #data = data_2018_2.append(data_2018_1).append(data_2017_4).append(data_2017_3).append(data_2017_2).append(data_2017_1)
        data = data_2018_2.append(data_2018_1)
    ...
    def download_profit_data(self, year=2017, quarter=3):
        ...
        try:
            ...
            data_2017_1.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
        except:
        ...
        try:
            ...
            data_2017_2.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
        except:
        ...
        try:
            ...
            data_2017_3.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        except:
        ...
        try:
            ...
            data_2017_4.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
        except:
            print('download profit_data_2017_4 failed')

        try:
            data_2018_1 = ts.get_profit_data(2018, 1)
            print('Now downloading profit_data_2018_1, total records:' + str(len(data_2018_1)))
            filename = 'profit_data_2018_1'
            data_2018_1.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        except:
            print('download profit_data_2018_1 failed')

        try:
            data_2018_2 = ts.get_profit_data(2018, 2)
            print('Now downloading profit_data_2018_2, total records:' + str(len(data_2018_2)))
            filename = 'profit_data_2018_2'
            data_2018_2.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
        except:
            print('download profit_data_2018_2 failed')
    ...
    def download_share_profit_data(self, year='2017'):
        ...
        try:
            ...
            share_data_2016.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        except:
        ...
        try:
            ...
            share_data_2017.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
        except:
    ...
    def download_new_stocks(self):
        ...
        if new_stocks is not None:
            ...
            filename = 'new_stocks'
            new_stocks.to_excel(self.working_folder + filename + '.xlsx', encoding='GBK')
        else:
            warning_code = pd.DataFrame(
    ...
    def download_ST_classified(self):
        ...
        if ST_classified is not None:
            ...
            ST_classified.to_excel(self.working_folder  + filename + '.xlsx', encoding='GBK')
        else:
            warning_code = pd.DataFrame(
    ...
    def download_stock_fundamentals(self, year=2010,end=2018):
        """
        """
    ...
    def format_stock_fundamentals(self, year=1990,end=2018):
        """
        :param year: 
        :return: 
        """
        ...
        while year <= end:
            for quarter in self.quarter_list:
                ...
                try:
                    ...
                    report_data['quarter'] = str(year) + '-' + str(quarter)
                    report_data['code'] = report_data['code'].map(lambda x: str(x).zfill(6))
                    stock_basics['code'] = stock_basics['code'].map(lambda x: str(x).zfill(6))
                    report_data = pd.merge(report_data, stock_basics, how='left', on=['code'])
    ...
    def merge_stock_fundamentals(self, year=1990,end=2018):
        ...
        while year <= end:
            for quarter in self.quarter_list:
                ...
                try:
                    ...
                    #stock_fundamentals['quarter'] = str(year) + '-' + str(quarter)
        ...
        stock_fundamentals_all.drop_duplicates(
            subset=['code', 'report_date'], keep='first', inplace=True)
        filename = 'stock_fundamentals_all'
