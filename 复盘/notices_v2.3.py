...
def sina_news_notification(bot):
    ...
    filename = 'notices'
    notices_last.to_excel('/Users/yanghui/Documents/tushare/Notices/' + filename + '_' + today_ISO + '.xlsx',
                          encoding='GBK')
    return
...
def M0_notification(bot):
    ...
    filename = '成交量M0占比'
    try:
        M0_last = pd.read_excel('/Users/yanghui/Documents/sina/' + filename + '.xlsx')
    except:
    ...
    if now >= M0_sched_Timer and M0_last_msg_sent_date != today_ISO:
        try:
            ...
            if len(index_sh) > 0 and len(index_sz) > 0:
                ...
                M0_sina = pd.read_excel('/Users/yanghui/Documents/sina/' + filename + '.xlsx')
                ##################################
                M0 = M0_sina.iloc[0, 5]
                ...
                filename = '成交量M0占比'
                M0_last.to_excel(
                    '/Users/yanghui/Documents/sina/' + filename + '.xlsx',
                    encoding='GBK')
            else:
...
def M1_notification(bot):
    ...
    filename = '成交量M1占比'
    try:
        M1_last = pd.read_excel('/Users/yanghui/Documents/sina/' + filename + '.xlsx')
    except:
    ...
    if now >= M1_sched_Timer and M1_last_msg_sent_date != today_ISO:
        try:
            ...
            if len(index_sh) > 0 and len(index_sz) > 0:
                ...
                filename = '货币供应量_宏观数据_新浪财经'
                M1_sina = pd.read_excel('/Users/yanghui/Documents/sina/' + filename + '.xlsx')
                ##################################
                M1 = M1_sina.iloc[0, 3]
