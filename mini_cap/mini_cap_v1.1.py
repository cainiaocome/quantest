...
def before_trading(context, bar_dict):
    ...
    fundamental_df = context.fundamentals_all[(context.fundamentals_all.date == context.prev_calendar_date) &
                                              ...
                                              (context.fundamentals_all.open < 5)
                                              ]
    ...
    # 过滤上市小于30天的股票
    for stk in context.stocks:
        market_dates = len(context.fundamentals_all[(context.fundamentals_all.date <= context.prev_calendar_date) &
...
def handle_bar(context, bar_dict):
    ...
    for stk in context.portfolio.positions:
        ...
        # 去除涨幅大于100%并且跌幅大于5%的股票
        for stk in context.portfolio.positions:
            position_percent = (
                                   context.portfolio.positions[stk].market_value - context.portfolio.positions[
                                       stk].bought_value) / context.portfolio.positions[stk].bought_value
            if position_percent >= 1 or position_percent <= -0.05:
